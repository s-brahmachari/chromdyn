...
class ChromatinTrajectory:
    def __init__(self, filename: str = None):
        ...
        self.dictChromSeq = {}
        self.topology = None
        self.box_vectors = None # shape: (Nframes, 3, 3)
...
def load(self, filename):
    ...
    if file_extension == ".ndb":
        filename = ndb2cndb(self, filename=f_name)   
    ...
    # Identify frame keys (digits) and sort them to ensure correct order
    frame_keys = sorted([key for key in self.cndb.keys() if key.isdigit()], key=int)
    self.Nframes = len(frame_keys)
    
    # --- 4. Load Box Vectors (New Feature) ---
    # We check the first frame to see if 'box' attribute exists
    self.box_vectors = None
    if self.Nframes > 0 and 'box' in self.cndb[frame_keys[0]].attrs:
        # Pre-allocate array: (Nframes, 3, 3)
        # OpenMM box vectors are 3 vectors (a, b, c) of 3 coordinates (x, y, z)
        self.box_vectors = np.zeros((self.Nframes, 3, 3))
...
def xyz(self, frames=[0, None, 1], beadSelection=None, XYZ=[0, 1, 2]):
    ...
    # check frames number
    if frames[1] is None:
        frames[1] = self.Nframes +1
        
    if frames[1] == frames[0]:  
        frames[1] += 1
    #print(f"Frame range: From {frames[0]} to {frames[1]} with step =  {frames[2]}")
