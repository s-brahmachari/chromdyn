...
nrep=12
data_home=/work/cms16/sb95/SAW_globule_extension_const_ext_k0.1_fixed_temp
# rm -r $data_home
mkdir -p -v $data_home
...
if [[ -z "$out_str" ]]; then
    out_str=$"python run_force_ext_const_ext.py -N ${N} -chi ${chi} -temp ${temp} -Nrep ${nrep} -output ${save_folder} -z ${z}"
else
    out_str+=$'\n'"python run_force_ext_const_ext.py -N ${N} -chi ${chi} -temp ${temp} -Nrep ${nrep} -output ${save_folder} -z ${z}"
fi
