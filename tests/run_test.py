...
# detailed testing of various features
class run_tests:
    def __init__(self):
    ...
    def remove_force(self, platform="CPU"):
        ...
        self.logger.info("-" * 80)
        self.logger.info(
            f"Running {platform} simulations to test force removal during sims"
        )
        self.logger.info("-" * 50)
    ...
    def bad_solvent(self, platform="CPU"):
        self.logger.info("-" * 80)
        self.logger.info(
            f"Running {platform} simulations to test type-to-type interactions"
        )
        self.logger.info("-" * 50)
        self.logger.info(f"{'N':<8} {'Rad. Gyr.':^20}")
        self.logger.info(f"{'':<8} {'expected':^10} {'sim':^10}")
        ...
        for Np in [50, 100, 500]:
            ...
            sim.force_field_manager.add_type_to_type_interaction(
                interaction_matrix, type_labels, rc=1.75
            )
